...
class TestCreateUserProfile:
    ...
    @pytest.fixture
    def setup_use_case(self, setup_repository: Repository) -> CreateUserProfileUseCase:
        return CreateUserProfileUseCase(setup_repository)


    def test_create_user_profile_success(self, setup_user_profile_input: CreateUserProfileInput, setup_repository: Repository,
                                         setup_use_case: CreateUserProfileUseCase) -> None:
        # Assign
        mocked_input = setup_user_profile_input
        repository = setup_repository
        # Act
        created_user = setup_use_case.execute(userDataInput=setup_user_profile_input)
